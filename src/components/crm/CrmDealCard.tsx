...
export function CrmDealCard({ deal, stageColor, onClick, monthlyAvg, showHistorico }: CrmDealCardProps) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      className="group relative cursor-pointer rounded-lg border bg-card p-3 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5 hover:border-primary/30 min-h-[44px]"
    >
      <div className="absolute left-0 top-0 h-full w-1 rounded-l-lg" style={{ backgroundColor: stageColor }} />
      <div className="ml-2 space-y-2">
        <div className="flex items-start justify-between">
          <h4 className="text-sm font-semibold leading-tight tracking-tight">{deal.title}</h4>
          <GripVertical className="h-4 w-4 text-muted-foreground/40 opacity-0 group-hover:opacity-100 transition-opacity" />
        </div>
        {deal.companies && (
          <p className="text-xs text-muted-foreground font-light">{deal.companies.name}</p>
        )}
        {showProgress && (
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          {!showProgress && potencial > 0 && (
            <span className="flex items-center gap-1 font-light text-foreground">
