CREATE OR REPLACE FUNCTION public.recompute_seguimiento_ventas(_company_id uuid, _ev empresa_vendedora) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $function$
DECLARE
  ...
  v_act_ua date; v_act_ta int:=0; v_act_ut date; v_act_tt int:=0;
...
BEGIN
  ...
  SELECT MAX(activity_date)::date, COUNT(*) INTO v_act_ua, v_act_ta FROM public.crm_activities WHERE company_id = _company_id;
  SELECT MAX(completed_at)::date, COUNT(*) INTO v_act_ut, v_act_tt FROM public.crm_tasks WHERE company_id = _company_id AND completed = true;
  v_act_ultima := GREATEST(v_act_ua, v_act_ut);
  IF v_act_ultima IS NULL THEN v_act_ultima := COALESCE(v_act_ua, v_act_ut); END IF;
  v_act_total := COALESCE(v_act_ta,0) + COALESCE(v_act_tt,0);
  v_dias_act := CASE WHEN v_act_ultima IS NOT NULL THEN (now()::date - v_act_ultima) END;
  SELECT COUNT(*) INTO v_act_activas FROM public.crm_tasks WHERE company_id = _company_id AND completed = false;
  SELECT MIN(due_date) INTO v_prox_ts FROM public.crm_tasks WHERE company_id = _company_id AND completed = false AND due_date IS NOT NULL AND due_date >= now();
  ...
  IF v_promedio > 0 THEN v_ritmo := (v_acum_mes / (v_promedio * v_month_frac)) * 100; END IF;
  IF v_tiene THEN
...
DO $$ DECLARE r record; BEGIN FOR r IN SELECT company_id, empresa_vendedora FROM public.seguimiento_ventas LOOP PERFORM public.recompute_seguimiento_ventas(r.company_id, r.empresa_vendedora); END LOOP; END $$;