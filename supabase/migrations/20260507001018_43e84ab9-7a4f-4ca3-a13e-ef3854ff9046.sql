CREATE OR REPLACE FUNCTION public.resolve_template_placeholders(_documento_id uuid DEFAULT NULL::uuid, _contacto_id uuid DEFAULT NULL::uuid, _pago_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    '{fecha_entrega_real}',         COALESCE(to_char(COALESCE(v_entrega.fecha_entrega_real, v_doc.fecha_entrega_real), 'DD/MM/YYYY'), ''),
    '{estatus_entrega}',            COALESCE(v_doc.estatus_entrega_corporativa::text, ''),
    '{acuse_url}',                  COALESCE(v_acuse_url, ''),
    '{orden_compra_url}',           COALESCE(v_oc_url, ''),
    '{url_documento}',              COALESCE(v_doc_url, v_doc.pdf_url, ''),
    '{liga_documento}',             COALESCE(v_doc_url, v_doc.pdf_url, ''),
    '{url_factura_pdf}',            COALESCE(v_doc.pdf_url, ''),
    ...
    '{tipo_pago}',                  COALESCE(v_pago.tipo_pago::text, ''),
