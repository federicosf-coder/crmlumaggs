-- Recalcular estatus_factura sin la categoría 'parcial'
...
-- Actualizar recompute_documento_cobranza para no usar 'parcial' en estatus_factura
    ...
    ELSIF v_venc IS NOT NULL AND v_venc < CURRENT_DATE THEN
      UPDATE public.documentos
        SET saldo_pendiente_cobranza = v_saldo,
            estado_cobranza = CASE WHEN v_aplicado > 0
                                   THEN 'parcial'::public.estado_cobranza_doc
                                   ELSE 'vencida'::public.estado_cobranza_doc END,
...
-- Reclasificar facturas existentes que estén en 'parcial'
UPDATE public.documentos
SET estatus_factura = CASE
  WHEN fecha_vencimiento IS NOT NULL AND fecha_vencimiento < CURRENT_DATE
    THEN 'vencida'::public.estatus_factura
  ELSE 'vigente'::public.estatus_factura
END
WHERE tipo_documento = 'factura'
  AND estatus_factura = 'parcial'::public.estatus_factura;
