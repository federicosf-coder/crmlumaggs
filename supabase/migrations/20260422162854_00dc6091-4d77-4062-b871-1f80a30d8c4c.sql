...
-- Merge companies: reasigna relaciones del duplicado al principal, completa campos vacíos, elimina duplicado.
...
BEGIN
  ...
  -- Reasignar FKs de tablas dependientes
  ...
  -- Junction tables: insertar al principal si falta y borrar del duplicado
  ...
  -- Completar campos vacíos del principal con los del duplicado
...
BEGIN
  ...
  UPDATE crm_deals SET contact_id = _primary_id WHERE contact_id = _duplicate_id;
  GET DIAGNOSTICS v = ROW_COUNT; counts := counts || jsonb_build_object('crm_deals', v);
