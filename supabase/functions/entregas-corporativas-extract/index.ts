...
const PROMPT = `Esta es una imagen o PDF de un calendario de entregas de un cliente corporativo. Tiene formato de matriz: cada fila es un producto (con código Chevron y nombre), y las columnas son fechas de entrega con la cantidad a entregar en cada celda. Extrae SOLO este JSON sin texto adicional ni markdown: {"entregas":[{"codigo":"string","nombre_producto":"string","fecha":"YYYY-MM-DD","cantidad":0}]}. Genera una entrada por cada combinación producto+fecha donde la cantidad sea mayor a 0 — ignora las celdas en cero o vacías. Las fechas suelen venir como 'DD-Mmm' en el encabezado de columna (ej. '10-Aug'); conviértelas a formato YYYY-MM-DD usando el año que corresponda según el contexto del documento (busca un año explícito en el título o encabezado; si no lo encuentras, usa el año actual). El código de producto suele ser el primer valor de cada fila (columna 'CHEVRON CODE' o similar).`;
