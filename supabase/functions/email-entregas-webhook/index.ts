...
Estos documentos suelen venir en dos formatos posibles:

(1) "ORDEN DE COMPRA": el número de pedido está en el campo "Num. de Orden". Los productos están en una tabla con columnas LINEA/ARTICULO/DESCRIPCION/CANTIDAD/UDM/PRECIO UNITARIO — usa ARTICULO como codigo, DESCRIPCION como nombre_producto, CANTIDAD como cantidad. Este documento suele listar varias direcciones de planta como catálogo de referencia (ej. "MTK APODACA", "MTK MEXICALI") — estas NO indican necesariamente cuál aplica a esta orden específica, así que en este tipo de documento deja "lugar_entrega" en null salvo que sea evidente que solo aplica una.

(2) "Order Confirmation" (en inglés, aunque el correo esté en español — a veces viene nombrado como archivo "Remisión" pero el contenido real es un Order Confirmation): el número de pedido real está en el campo "Purchase order number" — NUNCA uses el campo "Order number" como numero_pedido, ese es un folio interno distinto de Chevron. La dirección de entrega real y confirmada está en el bloque "Ship-To-Party" (usa el nombre de planta + la dirección completa que ahí aparezca, ej. "MTK MEXICALI — Carretera a San Luis # 950, Col. Gonzalez Ortega, 21397 Mexicali, México"). La fecha de entrega solicitada está en el campo "Requested delivery date" (formato DD-MMM-YYYY como "23-AUG-2026"; conviértela a YYYY-MM-DD). Los productos están en una tabla con columnas Item/Material/Description/Ord.Qty/UOM/Unit Price/Net Price — usa Material como codigo, Description como nombre_producto, Ord.Qty como cantidad, y usa la "Requested delivery date" como fecha de cada línea.

En ambos casos, lo más importante es identificar correctamente PARA DÓNDE (lugar_entrega) y CUÁNDO (fecha) se solicita cada entrega — prioriza la precisión de estos dos datos sobre cualquier otro detalle.
